...
# Data Augmentation is done by randomly flipping every image for both the y and x axes before using them for training.
...
def get_image_batch(data, idx, BATCH_SIZE, r_x=0, r_y=0):
    ...
    for i in range(BATCH_SIZE):
        if r_x == 1:
            sub_data[i] = flip_image_x(sub_data[i])
        if r_y == 1:
            sub_data[i] = flip_image_y(sub_data[i])
    return sub_data
